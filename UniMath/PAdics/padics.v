...
Definition quotientprecarry ( m : hz ) ( is : hzneq 0 m )
  ...
  fun x : nat => hzquotientmod m is ( precarry m is a x ).

Lemma quotientandtimesrearrangel ( m : hz ) ( is : hzneq 0 m ) ( x y : hz ) :
...
Lemma precarryandtimesl ( m : hz ) ( is: hzneq 0 m )
  ( a b : fpscommrng hz ) ( n : nat ) :
  hzquotientmod m is ( precarry m is (a * b ) n ) ~>
  ( ( quotientprecarry m is a * b ) n +
...
Proof.
  ...
  - unfold precarry.
    ...
    change ( ( quotientprecarry m is a * b ) 0%nat ) with
    ( hzquotientmod m is ( a 0%nat ) * b 0%nat ).
    rewrite quotientandtimesrearrangel.
    change ( ( carry m is a * b ) 0%nat ) with
    ( hzremaindermod m is ( a 0%nat ) * b 0%nat ).
    apply idpath.
  - change ( precarry m is ( a * b ) ( S n ) ) with
    ( ( a * b ) ( S n ) +
      hzquotientmod m is ( precarry m is ( a * b ) n ) ).
    rewrite IHn.
    rewrite <- ( rngassoc1 hz ).
    assert ( ( ( a * b ) ( S n ) +
               ( quotientprecarry m is a * b ) n ) ~>
      ( @op2 ( fpscommrng hz ) ( precarry m is a ) b ) ( S n ) ) as f.
    { change ( ( a * b ) ( S n ) )
     with ( natsummation0 ( S n ) ( fun x : nat => a x * b ( minus ( S n ) x ) ) ).
     change ( ( quotientprecarry m is a * b ) n ) with
     ( natsummation0 n ( fun x : nat =>
         quotientprecarry m is a x * b ( minus n x ) ) ).
     rewrite natsummationplusshift.
     change ( ( @op2 ( fpscommrng hz ) ( precarry m is a ) b ) ( S n ) ) with
     ( natsummation0 ( S n ) ( fun x : nat =>
         ( precarry m is a ) x * b ( minus ( S n ) x ) ) ).
    ...
    rewrite f.
    rewrite hzquotientmodandplus.
    change ( @op2 ( fpscommrng hz ) ( precarry m is a ) b ) with
    ( fpstimes hz ( precarry m is a ) b ).
    rewrite ( hzquotientandfpstimesl m is ( precarry m is a ) b ).
    change ( @op2 ( fpscommrng hz ) ( carry m is a ) b ) with
    ( fpstimes hz ( carry m is a ) b ) at 1.
    unfold fpstimes at 1.
    unfold carry at 1.
    change (fun n0 : nat =>
              let t' := fun m0 : nat =>
                          b (n0 - m0)%nat in natsummation0 n0 (fun x : nat =>
                     (hzremaindermod m is (precarry m is a x) * t' x)%rng)) with
    ( carry m is a * b ).
    change ( ( quotientprecarry m is a * b ) ( S n ) ) with
    ( natsummation0 ( S n ) ( fun i : nat =>
        hzquotientmod m is ( precarry m is a i ) * b ( S n - i )%nat ) ).
    rewrite 2! hzplusassoc.
    apply ( maponpaths ( fun v : _ => natsummation0 ( S n ) ( fun i : nat =>
      hzquotientmod m is ( precarry m is a i ) * b ( S n - i )%nat ) + v ) ).
    change ( precarry m is ( carry m is a * b ) ( S n ) ) with
    ( ( carry m is a * b ) ( S n ) +
      hzquotientmod m is ( precarry m is ( carry m is a * b ) n ) ).
    change ((carry m is a * b) (S n) +
            hzquotientmod m is (precarry m is (carry m is a * b) n)) with
    ((carry m is a * b)%rng (S n) +
     hzquotientmod m is (precarry m is (carry m is a * b) n)%rng)%hz.
    rewrite ( hzquotientmodandplus m is
                                   ( ( carry m is a * b ) ( S n ) )
     ( hzquotientmod m is ( precarry m is ( carry m is a * b ) n ) ) ).
    change ( ( carry m is a * b ) ( S n ) ) with
    ( natsummation0 ( S n ) ( fun i : nat =>
    ...
    apply ( maponpaths ( fun v : _ =>
       ( hzquotientmod m is ( natsummation0 ( S n ) ( fun i : nat =>
            hzremaindermod m is
                   ( precarry m is a i ) * b ( S n - i )%nat ) ) ) + v ) ).
    apply ( maponpaths ( fun v : _ =>
        hzquotientmod m is
           ( hzquotientmod m is
              ( precarry m is ( carry m is a * b )%rng n ) ) + v ) ).
    apply maponpaths.
    apply ( maponpaths ( fun v : _ =>
    ...
    change ( hzremaindermod m is
                            (hzremaindermod m is (precarry m is a j) *
                             b ( minus ( S n ) j)) ) with
    ( hzremaindermod m is
                     (hzremaindermod m is (precarry m is a j) *
                      b (S n - j)%nat)%hz ).
...
Lemma carryandtimesl ( m : hz ) ( is : hzneq 0 m )
  ( a b : fpscommrng hz ) :
  carry m is ( a * b ) ~> carry m is ( carry m is a * b ).
Proof.
  intros.
  assert ( forall n : nat, carry m is ( a * b ) n ~>
                          carry m is ( carry m is a * b ) n ) as f.
  { intros n.
    destruct n.
    - unfold carry at 1 2.
      change ( precarry m is ( a * b ) 0%nat ) with
      ( a 0%nat * b 0%nat ).
      change ( precarry m is ( carry m is a * b ) 0%nat ) with
      ( carry m is a 0%nat * b 0%nat ).
      unfold carry.
      change (hzremaindermod m is (precarry m is a 0) * b 0%nat) with
      (hzremaindermod m is (precarry m is a 0) * b 0%nat )%hz.
      rewrite ( hzremaindermodandtimes m is
        ( hzremaindermod m is ( precarry m is a 0%nat ) ) ( b 0%nat ) ).
      rewrite hzremaindermoditerated.
      rewrite <- hzremaindermodandtimes.
      change ( precarry m is a 0%nat ) with ( a 0%nat ).
      apply idpath.
    - unfold carry at 1 2.
      change ( precarry m is ( a * b ) ( S n ) ) with
      ( ( a * b ) ( S n ) + hzquotientmod m is ( precarry m is ( a * b ) n ) ).
      rewrite precarryandtimesl.
      rewrite <- ( rngassoc1 hz ).
      rewrite hzremaindermodandplus.
      assert ( hzremaindermod m is
          ( ( a * b ) ( S n ) + ( quotientprecarry m is a * b ) n ) ~>
        hzremaindermod m is ( ( carry m is a * b ) ( S n ) ) ) as g.
      { change ( hzremaindermod m is ( ( natsummation0 ( S n ) ( fun u : nat =>
        ...
        assert ( hzremaindermod m is ( natsummation0 n ( fun x : nat =>
                 ...
                 hzremaindermod m is (natsummation0 n ( fun x : nat =>
                   carry m is a ( S x ) * b ( minus ( S n ) ( S x ) ) ) ) ) as h.
        { rewrite hzqrandnatsummation0r.
          rewrite ( hzqrandnatsummation0r m is ( fun x : nat =>
                          carry m is a ( S x ) * b ( minus ( S n ) ( S x ) ) ) ).
          apply maponpaths.
          apply natsummationpathsupperfixed.
          intros j p.
          unfold quotientprecarry.
...
(* the following command takes overly long on Oct. 30, 2017:
          change (a (S j) * b ( minus n j) +
                  hzquotientmod m is (precarry m is a j) * b ( minus n j)) with
                 (a (S j) * b ( minus n j) +
                  hzquotientmod m is (precarry m is a j) * b ( minus n j) )%hz.
 *)
          intermediate_path (hzremaindermod m is ((a (S j) * b ( minus n j) +
                   hzquotientmod m is (precarry m is a j) * b ( minus n j) )%hz)).
          + exact (idpath _).
          + rewrite <- ( hzrdistr ( a ( S j ) )
                                 ( hzquotientmod m is ( precarry m is a j ) )
                                 ( b ( minus n j ) ) ).
            rewrite hzremaindermodandtimes.
            change ( hzremaindermod m is
                     ...
                     hzremaindermod m is (carry m is a (S j) * b(minus n j)) )%rng.
        ...
        rewrite ( hzremaindermodandplus m is _
                    ( hzremaindermod m is ( precarry m is a 0%nat ) *
                                            b ( minus ( S n ) 0%nat ) ) ).
        rewrite hzremaindermodandtimes.
        rewrite hzremaindermoditerated.
        rewrite <- hzremaindermodandtimes.
...
Lemma carryandtimesr ( m : hz ) ( is : hzneq 0 m )
  ( a b : fpscommrng hz ) :
  carry m is ( a * b ) ~> carry m is ( a * carry m is b ).
Proof.
  intros.
...
Lemma carryandtimes ( m : hz ) ( is : hzneq 0 m )
  ( a b : fpscommrng hz ) :
  carry m is ( a * b ) ~> carry m is
                           ( carry m is a * carry m is b ).
Proof.
  intros.
  rewrite carryandtimesl.
...
Lemma rngcarryequiv ( m : hz ) ( is : hzneq 0 m ) :
  @rngeqrel ( fpscommrng hz ).
Proof.
  intros.
  split with ( carryequiv m is ).
  split.
  - split.
    + intros a b c q.
      simpl.
...
Definition padicplus ( p : hz ) ( is : isaprime p ) :=
  @op1 ( commrngofpadicints p is ).

Definition padictimes ( p : hz ) ( is : isaprime p ) :=
  @op2 ( commrngofpadicints p is ).

(** * III. The apartness relation on p-adic integers *)

Definition padicapart0 ( p : hz ) ( is : isaprime p ) :
  hrel ( fpscommrng hz ) :=
  fun a b : _ =>
    ( hexists ( fun n : nat =>
...
Lemma padicapartiscomprel ( p : hz ) ( is : isaprime p ) :
  iscomprelrel ( carryequiv p ( isaprimetoneq0 is ) )
               ( padicapart0 p is ).
Proof.
  intros p is a a' b b' i j.
...
Definition padicapart1 ( p : hz ) ( is : isaprime p ) :
  hrel ( commrngofpadicints p is ) :=
  quotrel ( padicapartiscomprel p is ).

Lemma isirreflpadicapart0 ( p : hz ) ( is : isaprime p ) :
  isirrefl ( padicapart0 p is ).
Proof.
  intros. intros a f.
  simpl in f.
  assert hfalse as x.
...
Lemma issymmpadicapart0 ( p : hz ) ( is : isaprime p ) :
  issymm ( padicapart0 p is ).
Proof.
  intros. intros a b f.
  refine (hinhuniv _ f).
  intros u.
...
Lemma iscotranspadicapart0 ( p : hz ) ( is : isaprime p ) :
  iscotrans ( padicapart0 p is ).
Proof.
  intros. intros a b c f.
  refine (hinhuniv _ f).
  intro u.
  destruct u as [ n u ].
  intros P j.
...
Definition padicapart ( p : hz ) ( is : isaprime p ) :
  apart ( commrngofpadicints p is ).
Proof.
  intros.
  split with ( padicapart1 p is ).
  split.
  - unfold padicapart1.
    apply ( isirreflquotrel ( padicapartiscomprel p is )
                            ( isirreflpadicapart0 p is ) ).
  - split.
    + apply ( issymmquotrel ( padicapartiscomprel p is )
                            ( issymmpadicapart0 p is ) ).
    + apply ( iscotransquotrel ( padicapartiscomprel p is )
                               ( iscotranspadicapart0 p is ) ).
Defined.

Lemma precarryandzero ( p : hz ) ( is : isaprime p ) :
...
Lemma carryandzero ( p : hz ) ( is : isaprime p ) :
  carry p ( isaprimetoneq0 is ) 0 ~> 0.
Proof.
  intros.
  unfold carry.
...
Lemma precarryandone ( p : hz ) ( is : isaprime p ) :
...
Lemma carryandone ( p : hz ) ( is : isaprime p ) :
  carry p ( isaprimetoneq0 is ) 1 ~> 1.
Proof.
  intros.
  unfold carry.
...
Lemma padicapartandplusprecarryr ( p : hz ) ( is : isaprime p )
  ( a b c : fpscommrng hz ) ( n : nat )
  ( x : neq _ ( precarry p ( isaprimetoneq0 is )
...
Proof.
  ...
  apply ( padicapartandplusprecarryl p is a b c n x ).
Defined.
...
Lemma setquotprandpadicplus ( p : hz ) ( is : isaprime p )
  ( a b : fpscommrng hz ) :
  ( @op1 ( commrngofpadicints p is ) )
    ( setquotpr ( carryequiv p ( isaprimetoneq0 is ) ) a )
    ( setquotpr ( carryequiv p ( isaprimetoneq0 is ) ) b ) ~>
...
Lemma setquotprandpadictimes ( p : hz ) ( is : isaprime p )
  ( a b : fpscommrng hz ) :
  ( @op2 ( commrngofpadicints p is ) )
    ( setquotpr ( carryequiv p ( isaprimetoneq0 is ) ) a )
    ( setquotpr ( carryequiv p ( isaprimetoneq0 is ) ) b ) ~>
...
Lemma padicplusisbinopapart0 ( p : hz ) ( is : isaprime p )
  ( a b c : fpscommrng hz )
  ( u : padicapart0 p is ( a + b ) ( a + c ) ) :
  padicapart0 p is b c.
Proof.
  intros.
  refine (hinhuniv _ u).
  ...
  - destruct ( isdeceqhz ( carry p ( isaprimetoneq0 is ) b ( S k ) )
                         ( carry p ( isaprimetoneq0 is ) c ( S k ) ) )
      as [ l | r ].
    + apply ( padicapartandplusprecarryl p is a b c k ).
      intros j.
...
Lemma padicplusisbinopapartl ( p : hz ) ( is : isaprime p ) :
  isbinopapartl ( padicapart p is ) ( padicplus p is ).
Proof.
  intros.
  unfold isbinopapartl.
  assert ( forall x x' x'' : commrngofpadicints p is,
             isaprop ( ( pr1 ( padicapart p is ) )
                         ( padicplus p is x x' )
                         ( padicplus p is x x'' ) ->
                       ( ( pr1 ( padicapart p is ) ) x' x'' ) ) ) as int.
  { intros.
    apply impred.
    intros.
    apply ( pr1 ( padicapart p is ) ).
  }
  apply ( setquotuniv3prop _ ( fun x x' x'' =>
                                 hProppair _ ( int x x' x'' ) ) ).
  intros a b c.
  change (pr1 (padicapart p is)
              (padicplus p is
                         (setquotpr (rngcarryequiv p (isaprimetoneq0 is)) a)
                         (setquotpr (rngcarryequiv p (isaprimetoneq0 is)) b))
              (padicplus p is (setquotpr (rngcarryequiv p (isaprimetoneq0 is)) a)
...
Lemma padicplusisbinopapartr ( p : hz ) ( is : isaprime p ) :
  isbinopapartr ( padicapart p is ) ( padicplus p is ).
Proof.
  intros.
  unfold isbinopapartr.
  intros a b c.
  unfold padicplus.
  rewrite ( @rngcomm1 ( commrngofpadicints p is ) b a ).
  rewrite ( @rngcomm1 ( commrngofpadicints p is ) c a ).
  apply padicplusisbinopapartl.
Defined.

Lemma padicapartandtimesprecarryl ( p : hz ) ( is : isaprime p )
  ( a b c : fpscommrng hz ) ( n : nat )
  ( x : neq _ ( precarry p ( isaprimetoneq0 is )
                         ( carry p ( isaprimetoneq0 is ) a *
                           carry p ( isaprimetoneq0 is ) b ) n )
...
Lemma padictimesisbinopapart0 ( p : hz ) ( is : isaprime p )
  ( a b c : fpscommrng hz )
  ( u : padicapart0 p is ( a * b ) ( a * c ) ) :
  padicapart0 p is b c.
Proof.
  ...
  - set ( Q := ( fun o : nat =>
    ...
    + apply ( padicapartandtimesprecarryl p is a b c n ).
      intros j.
      assert hfalse as xx.
      { unfold neq in n'.
        apply n'.
...
Lemma padictimesisbinopapartl ( p : hz ) ( is : isaprime p ) :
  isbinopapartl ( padicapart p is ) ( padictimes p is ).
Proof.
  intros.
  unfold isbinopapartl.
  assert ( forall x x' x'' : commrngofpadicints p is,
  ...
  change (pr1 (padicapart p is)
              (padictimes p is
                          (setquotpr (carryequiv p (isaprimetoneq0 is)) a)
                          (setquotpr (carryequiv p (isaprimetoneq0 is)) b))
              (padictimes p is (setquotpr (carryequiv p (isaprimetoneq0 is)) a)
                          (setquotpr (carryequiv p (isaprimetoneq0 is)) c)) ->
          pr1 (padicapart p is) (setquotpr (carryequiv p (isaprimetoneq0 is)) b)
              (setquotpr (carryequiv p (isaprimetoneq0 is)) c)).
  unfold padictimes.
  rewrite 2! setquotprandpadictimes.
  rewrite 2! padicapartcomputation.
  intros j.
  apply ( padictimesisbinopapart0 p is a b c j ).
Defined.

Lemma padictimesisbinopapartr ( p : hz ) ( is : isaprime p ) :
  isbinopapartr ( padicapart p is ) ( padictimes p is ).
Proof.
  intros.
  unfold isbinopapartr.
  intros a b c.
  unfold padictimes.
  rewrite ( @rngcomm2 ( commrngofpadicints p is ) b a ).
  rewrite ( @rngcomm2 ( commrngofpadicints p is ) c a ).
...
Proof.
  intros.
  split with ( commrngofpadicints p is ).
  split with ( padicapart p is ).
  split.
  - split.
    + apply ( padicplusisbinopapartl p is ).
    + apply ( padicplusisbinopapartr p is ).
  - split.
    + apply ( padictimesisbinopapartl p is ).
    + apply ( padictimesisbinopapartr p is ).
Defined.

(** * IV. The apartness domain of p-adic integers and the Heyting field of p-adic numbers *)

Lemma precarryandzeromultl ( p : hz ) ( is : isaprime p )
  ( a b : fpscommrng hz ) ( n : nat )
...
Lemma precarryandzeromultr ( p : hz ) ( is : isaprime p )
  ( a b : fpscommrng hz ) ( n : nat )
  ( x : forall m : nat, natlth m n ->
...
Proof.
  ...
  rewrite ( ( @rngcomm2 ( fpscommrng hz ) )
              ( carry p ( isaprimetoneq0 is ) a )
              ( carry p ( isaprimetoneq0 is ) b ) ).
  apply ( precarryandzeromultl p is b a n x m y ).
Defined.
...
Lemma precarryandzeromult ( p : hz ) ( is : isaprime p )
  ( a b : fpscommrng hz ) ( k k' : nat )
...
Proof.
  intros.
  refine (hinhuniv _ ( primedivorcoprime p a is )).
  intros j. intros P i.
  apply i.
  destruct j as [ j0 | j1 ].
  - apply ii1.
...
Lemma hzremaindermodprimeandtimes ( p : hz ) ( is : isaprime p )
  ( a b : hz )
...
Proof.
  ...
  refine (hinhuniv _ ( primeandtimes p a b is i )).
  intro t.
  destruct t as [ t0 | t1 ].
...
Definition padiczero ( p : hz ) ( is : isaprime p ) :=
  @rngunel1 ( commrngofpadicints p is ).

Definition padicone ( p : hz ) ( is : isaprime p ) :=
  @rngunel2 ( commrngofpadicints p is ).

Lemma padiczerocomputation ( p : hz ) ( is : isaprime p ) :
  padiczero p is ~>
...
Lemma padiconecomputation ( p : hz ) ( is : isaprime p ) :
  padicone p is ~>
  setquotpr ( carryequiv p ( isaprimetoneq0 is ) )
            ( @rngunel2 ( fpscommrng hz ) ).
Proof.
...
Lemma padicintsareintdom ( p : hz ) ( is : isaprime p )
  ( a b : acommrngofpadicints p is ) : a # 0 -> b # 0 -> a * b # 0.
Proof.
  intros p is.
  assert ( forall a b : commrngofpadicints p is,
     isaprop ( ( pr1 ( padicapart p is ) ) a ( padiczero p is ) ->
               ( pr1 ( padicapart p is ) ) b ( padiczero p is ) ->
               ( pr1 ( padicapart p is ) ) ( padictimes p is a b )
                                           ( padiczero p is ) ) ) as int.
  { intros.
    apply impred.
    intros.
    apply impred.
  ...
  - destruct o.
    + rewrite <- carryandtimes.
      ...
      assert hfalse.
      { refine (hinhuniv _ ( hzremaindermodprimeandtimes p is
                                            ( a 0%nat ) ( b 0%nat ) v )).
        intros t.
        destruct t as [ t0 | t1 ].
        * unfold P in k'. unfold neq in k'.
    ...
    + intros v.
      ...
      rewrite ( precarryandzeromult p is a b 0%nat ( S o ) ) in v.
  ...
  - intros v.
    ...
    ( fpstimes hz ( carry p ( isaprimetoneq0 is ) a )
                  ( carry p ( isaprimetoneq0 is ) b ) ) in v.
    rewrite ( precarryandzeromult p is a b ( S k ) o ) in v.
    + rewrite hzqrand0q in v.
      rewrite hzplusr0 in v.
      assert hfalse.
      { set (aux := hzremaindermodprimeandtimes p is
...
Proof.
  intros.
  split with ( acommrngofpadicints p is ).
  split.
  - change ( ( pr1 ( padicapart p is ) )
               ( padicone p is )
               ( padiczero p is ) ).
    rewrite ( padiczerocomputation p is ).
    rewrite ( padiconecomputation p is ).
...
Definition padics ( p : hz ) ( is : isaprime p ) : afld :=
  afldfrac ( padicintegers p is ).
