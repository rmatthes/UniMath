...
Require UniMath.Induction.M.CategoryTheory_legacy.Core.Univalence.  (* double_transport *)
Require UniMath.Induction.M.CategoryTheory_legacy.Core.Functors.    (* functor_data_eq *)

(** Should be moved in Combinatorics/Graph.v,
    but it depends on code from Induction.M.CategoryTheory_legacy for now. *)
Definition graph_mor_eq {G H : pregraph} (p q : graph_mor G H)
           (e₀ : ∏ x : vertex G, onvertex p x = onvertex q x)
           (e₁ : ∏ x y (f : edge G x y),
                 UniMath.Induction.M.CategoryTheory_legacy.Core.Univalence.double_transport
                   (e₀ x) (e₀ y) (onedge p f) =
                 onedge q f)
  : p = q
  := UniMath.Induction.M.CategoryTheory_legacy.Core.Functors.functor_data_eq G H p q e₀ e₁.

Lemma isaprop_has_edgesets (G : pregraph)
  : isaprop (has_edgesets G).
Proof.
  apply UniMath.Induction.M.CategoryTheory_legacy.Core.Categories.isaprop_has_homsets.
Qed.
